...
Use `Customized Validation Rules` to solve this edge case. A `validator` function provided the 4th param called `source` that includes all the field values.
...
使用 `自定义校验规则` 来解决这个问题。一个 `validator` 方法提供了第 4 个参数：`source`，这个参数包含了当前所有的表单项的值。
